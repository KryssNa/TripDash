...
class view_hotel extends StatefulWidget {
  List<view_hotel> hotelList;

  const view_hotel({super.key});

  @override
  Future<State<view_hotel>> createState() async => view_hotel();
}

class view_hotel extends State<view_hotel>
    with TickerProviderStateMixin {
  DateTime startDate = DateTime.now();
  DateTime endDate = DateTime.now().add(const Duration(days: 5));

  List<view_hotel> hotelList = view_hotel().hotelList;
  ...
  Widget getAppBarUI() {
    return Container(
      decoration: BoxDecoration(
          color: view_hotel.buildLightTheme().backgroundColor,
          boxShadow: [
            BoxShadow(
                color: Colors.grey.withOpacity(0.2),
                offset: const Offset(0, 2),
      ...
      child: Padding(
        padding: EdgeInsets.only(
            top: MediaQuery.of(context as BuildContext).padding.top, left: 8, right: 8),
        child: Row(
          children: <Widget>[
            Container(
